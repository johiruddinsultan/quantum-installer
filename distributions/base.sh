...
function install_quantumx {
	_DIR=$(pwd)
	rm -rf "/tmp/quantumx/"
	git clone $quantumx_GIT_URL /tmp/quantumx
	cd /tmp/quantumx
	if [ -n "$quantumx_GIT_BRANCH" ]; then
		git checkout $quantumx_GIT_BRANCH
	else
	...
	if [ "$python" != "python3" ]; then
		sed -i "s:usr/bin/env python3:usr/bin/env python3.7:" quantumx_rpc
		sed -i "s:usr/bin/env python3:usr/bin/env python3.7:" quantumx_server
	fi
	$python -m pip install . --upgrade > /dev/null 2>&1
	if ! $python -m pip install . --upgrade; then
		_error "Unable to install quantumx" 7
	fi
	cd $_DIR
}
